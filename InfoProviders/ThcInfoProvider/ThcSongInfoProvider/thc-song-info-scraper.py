...
# try:
from InfoProviders.BasicInfoProvider.Model.BasicInfoModel import BasicInfoDb, BasicAlbum, BasicTrack
from InfoProviders.ThcInfoProvider.ThcSongInfoProvider.Model.ThcSongInfoModel import Track, Album, SaleSource, ProcessStatus, ThccDb
from InfoProviders.ThcInfoProvider.ThcQueryProvider.Model.QueryModel import QueryStatus, QueryData
from InfoProviders.CacheInfoProvider.Cache import cached
# except:
#     currentdir = os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))
#     parentdir = os.path.dirname(currentdir)
#     sys.path.insert(0, parentdir)
#     from BasicInfoProvider.Model.BasicInfoModel import BasicInfoDb, BasicAlbum, BasicTrack

def transform(list, transformer):
    t = []
    for v in list:
        t.append(transformer(v))
    return t
...
class ThWikiCc:
    ...
    ALBUM_INFO_ARGS_ARRAY = [
        "cover_char",
        "album_artist",
        "genre"
    ]

    ALBUM_INFO_ARGS_NO_RESOLVE = [
        "展会"
    ]
    ...
    TRACK_KNOWN_ARGS_IGNORE = [
        "乐器",
        "演奏阵列"
    ]

    TRACK_VALUE_SINGLE_ITEM = [
        "duration",
        "title"
    ]
    ...
    @staticmethod
    @cached(cache_id="thc", cache_dir="./InfoProviders/ThcInfoProvider/ThcSongInfoProvider/Cached")
    def get_source(url):
        ...
        for key, value in data.items():
            if key in ThWikiCc.ALBUM_INFO_PRELIM_ARGS:
                ...
                if (key in ThWikiCc.ALBUM_INFO_ARGS_ARRAY):
                    album_metadata[key] = list(filter(lambda x: x, value.split(ThWikiCc.FW_COMMA)))
                else:
                    album_metadata[key] = value
    ...
    def parse_album_info(data: Wikicode):
        template: Template = list(filter(lambda x: x.name == "同人专辑信息", data.filter_templates()))[0]
        if (template is None):
        ...
        for param in template.params:
            if (name := str(param.name).strip()) in ThWikiCc.ALBUM_INFO_PRELIM_ARGS:
                if (ThWikiCc.ALBUM_INFO_PRELIM_ARGS[name] is not None):
                    album_info[ThWikiCc.ALBUM_INFO_PRELIM_ARGS[name]] = str(param.value).strip()
            else:
        ...
        for track in data:
            ...
            for k, v in track.items():
                ...
                if k in ThWikiCc.TRACK_VALUE_SINGLE_ITEM:
                    fmt_track_info[k] = transform(v, transformer)[0]
                    continue

                fmt_track_info[k] = transform([r for i in v for r in i.split(ThWikiCc.FW_COMMA) if r], transformer)
        ...
        for idx, template in enumerate(track_templates, 1):
            ...
            for param in template.params:
                if (name := str(param.name).strip()) in ThWikiCc.TRACK_KNOWN_ARGS:
                    if ((trans_name := ThWikiCc.TRACK_KNOWN_ARGS[name]) is not None and
                            track.get(trans_name) is not None):
        ...
        for template in seller:
            ...
            for param in template.params:
                if ((name := (str(param.name).strip())) in ThWikiCc.SELLER_INFO_ARGS):
                    ...
                    if (not name.isdigit()):
                        print(f"Unknown seller info param: {name} in seller info: {seller_info}")
...
def import_data():
    ...
    for query in QueryData.select().where(QueryData.query_exact_result != None):
        ...
        album = Album(album_id=query.album_id, data_source=url, process_status=ProcessStatus.PENDING)
