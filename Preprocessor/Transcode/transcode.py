...
def mk_ffmpeg_cmd(path, out_path, use_libfdk_aac):
    ...
    return ["-i", f"\'{path}\'", "-c:a", lib, "-vbr", "3", f"\'{out_path}\'", "-y"]
...
def process_one(filelist, key, ffmpeg_path, use_libfdk_aac):
    ...
    try:
        ...
        # TODO: would also have to check if the result files exists and is not empty
        if (os.path.isfile(dst) or os.path.getsize(dst) < 1):
    ...
    for root, dirs, files in os.walk(root):
        for file in files:
            if file.endswith(".flac"):
                ...
                out_name = os.path.basename(path).replace(".flac", ".ts")
...
def main():
    ...
    filelist = load_file_list(file_list)
